...
This API receives the mermaid diagram definition as the input, parses the text and validates it. It returns true if the diagram is valid, false otherwise.

** Example **

```ts
import { validateMermaid } from "@excalidraw/mermaid-to-excalidraw";
const isValidMermaidDiagramDefinition = await validateMermaid(mermaidSyntax)
```
